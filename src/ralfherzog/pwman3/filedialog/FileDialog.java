package ralfherzog.pwman3.filedialog;
...
public class FileDialog extends ListActivity {
	 ...
	 * 
	 * @see {@link FileDialogSelectionMode}
	 */
	...
	private int selectionMode = FileDialogSelectionMode.MODE_CREATE;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		selectionMode = getIntent().getIntExtra(SELECTION_MODE, FileDialogSelectionMode.MODE_CREATE);

		formatFilter = getIntent().getStringArrayExtra(FORMAT_FILTER);

		canSelectDir = getIntent().getBooleanExtra(CAN_SELECT_DIR, false);

		if (selectionMode == FileDialogSelectionMode.MODE_OPEN) {
	...
	private void getDirImpl(final String dirPath) {
		...
		myPath.setText(getText(R.string.file_browser_location) + ": " + currentPath);

		if (!currentPath.equals(ROOT)) {

			item.add(ROOT);
			addItem(ROOT, R.drawable.file_browser_folder);
			path.add(ROOT);

			item.add("../");
			addItem("../", R.drawable.file_browser_folder);
		...
		for (String dir : dirsMap.tailMap("").values()) {
			addItem(dir, R.drawable.file_browser_folder);
		}

		for (String file : filesMap.tailMap("").values()) {
			addItem(file, R.drawable.file_browser_file);
		}
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		if (file.isDirectory()) {
			...
			if (file.canRead()) {
				...
				new AlertDialog.Builder(this).setIcon(R.drawable.file_browser_icon)
						.setTitle("[" + file.getName() + "] " + getText(R.string.file_browser_cant_read_folder))
